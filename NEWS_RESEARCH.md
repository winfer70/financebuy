...
1. **Server B (Kali)** runs a single-threaded Python worker (`server-b-worker/worker.py`) that:
...
- Approach 1 is simpler but adds operational complexity (another process to manage). Approach 2 is a significant refactor of the worker.
...
If revisited in the future, the best approach would be to monitor only 2-3 high-quality public channels (e.g., Unusual Whales, a single reputable analyst) as a supplementary signal, not a primary news source.
