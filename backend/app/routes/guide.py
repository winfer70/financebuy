...
_SYSTEM_PROMPT = (
    ...
    "KPI cards strip for at-a-glance portfolio metrics, "
    "asset ribbon showing portfolio weight by asset type (Stocks/Crypto/ETFs/Physical) with day P&L colour intensity, "
    "concentration chips highlighting over-weight positions, "
    "QuickSell drawer — click any position tile to open a right-edge slide-in panel with ticker, quantity, and average cost; place a sell order directly from Dashboard\n"
    ...
    "Trade History tab showing complete buy/sell trade log with entry/exit prices and P&L per trade plus aggregate trade stats, "
    "delete individual trade records from Trade History tab, "
    "cash balance adjustment modal in Portfolio Manager header to add or subtract cash from portfolio, "
    ...
    "stale data banner warning when price quotes are more than 5 minutes old\n"
    ...
    "keyboard shortcuts system — press '?' to open shortcuts modal, "
    "global navigation shortcuts use two-key sequences starting with 'g': g+d (Dashboard), g+w (Watchlist), "
    "g+p (Portfolio Manager), g+t (Trading), g+n (News), g+c (Charts) with 1500ms window, "
    "shortcuts disabled when input/textarea/select has focus\n"
    ...
    "- Portfolio Rules Engine: Accessible from the RULES tab inside Portfolio Manager. "
    "Analyses every position against 8 configurable rules and generates alerts with severity "
    "levels (INFO, WARNING, CRITICAL). Rules include: House Money (position has returned "
    "original capital — suggests locking gains), Stop Proximity (price within 5% = warning, "
    "within 2% = critical), Semi-Cap Concentration (semiconductor position exceeds portfolio "
    "allocation cap), Bucket Balance (checks over/underweight bucket tags like Core/Growth/"
    "Speculative), Time Stop (position held longer than expected without reaching profit target), "
    "Analyst Consensus (price significantly above analyst price target), Fundamentals Health "
    "(debt-to-equity, gross margin trend, insider activity), Pre-Earnings (alert 1-2 business "
    "days before scheduled earnings). Click RUN RULES to trigger analysis or select a schedule "
    "(on demand, auto market hours, auto end of day). Alerts can be filtered by severity and "
    "state, and marked as SNOOZED or ACTIONED once reviewed\n"
