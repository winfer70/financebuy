...
## Summary
- 3019 nodes · 10290 edges · 138 communities detected
- Extraction: 31% EXTRACTED · 69% INFERRED · 0% AMBIGUOUS · INFERRED: 7108 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## God Nodes (most connected - your core abstractions)
1. `User` - 190 edges
2. `PortfolioPosition` - 122 edges
...
Cohesion: 0.02
Nodes (323): _apply_stage_result(), _build_soft_stop_report(), _check_condition(), _check_soft_stops(), evaluate_one_soft_stop(), evaluate_price_alerts(), _fetch_daily_close(), _fetch_price() (+315 more)

### Community 1 - "Community 1"
Cohesion: 0.04
Nodes (188): admin_check(), list_audit_logs(), list_users(), lock_account(), lock_user(), admin.py — Admin-only routes for TickerTap.  Provides endpoints for user and a, Lock or unlock a brokerage account atomically.      Eliminates duplication bet, List all registered users, ordered newest-first.      Args:         db: Async (+180 more)
...
Cohesion: 0.22
Nodes (178): LLMTranspileError, Raised when LLM translation or validation fails., MarketDataProvider, BacktestResult, Notification, PaperTrade, PaperTradeEquitySnapshot, PaperTradePosition (+170 more)

### Community 5 - "Community 5"
Cohesion: 0.03
Nodes (109): concern_level(), ConsensusBrief, format_insider_telegram(), format_news_digest(), format_volume_compact(), is_c_suite(), OwnerPattern, _plan_line() (+101 more)

### Community 6 - "Community 6"
Cohesion: 0.03
Nodes (111): AuthOut, create_access_token(), decode_access_token(), hash_password(), login(), LoginIn, auth.py — Core authentication utilities for TickerTap.  Provides password hash, Raise on startup if the JWT secret is still the insecure default.      Called (+103 more)

### Community 7 - "Community 7"
Cohesion: 0.07
Nodes (91): _aggregate_bars(), ExchangeRateResponse, _fetch_earnings_dates_set(), _fetch_events(), _fetch_exchange_rates(), _fetch_fundamentals(), _fetch_ohlcv(), _fetch_ohlcv_interval() (+83 more)
...
Cohesion: 0.06
Nodes (92): _fetch_map(), cik_ticker_map.py — CIK -> ticker resolution via SEC's free company_tickers.json, Best-effort CIK -> ticker. None if unresolvable (private issuer,     stale cache, resolve_ticker(), _change_pct_to_score(), _check_article_outcomes(), create_scoring_rules(), export_training_data() (+84 more)

### Community 10 - "Community 10"
Cohesion: 0.04
Nodes (78): cleanup_dead_letters(), _connect(), enqueue(), enqueue_batch(), get_pending(), increment_retries(), init_db(), mark_done() (+70 more)

### Community 11 - "Community 11"
Cohesion: 0.04
Nodes (67): _delete_manual_positions(), _extract_cash_balance(), _flatten_position(), _get_account_overview(), _get_int_account(), _get_portfolio(), _get_products_info(), _get_stop_orders() (+59 more)
...
Cohesion: 0.13
Nodes (25): _mdy_to_iso(), parse_form144_xml(), form144_edgar.py — Parse SEC Form 144 (Notice of Proposed Sale) atom + XML.  An, One proposed-sale notice per filing (unlike Form 4's multi-row shape).      Retu, EDGAR's ownership XML dates are MM/DD/YYYY, unlike Form 4's ISO dates., _accession_from_href(), _accession_from_title(), _doc_level_10b5() (+17 more)
...
## Knowledge Gaps
- **772 isolated node(s):** `env.py — Alembic migration environment for TickerTap.  DATABASE_URL environmen`, `Run migrations in 'offline' mode (no live DB connection required).      Genera`, `Run migrations against a live database connection.`, `initial  Revision ID: 0001_initial Revises: Create Date: 2026-01-09 00:00:00`, `password reset tokens  Revision ID: 0002_password_reset_tokens Revises: 0001_` (+767 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 105`** (2 nodes): `dependencies.py`, `dependencies.py — Shared FastAPI dependencies for TickerTap.  Re-exports the c`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 106`** (2 nodes): `.get_quote()`, `Fetch the latest quote for *symbol*.          Args:             symbol: Ticke`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 107`** (2 nodes): `.get_supported_intervals()`, `Return the provider's supported interval list.          Returns:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 108`** (2 nodes): `Return the list of candle intervals this provider supports.          Returns:`, `.get_supported_intervals()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 109`** (2 nodes): `Synchronous symbol search via yfinance.          Args:             query: Fre`, `._search_sync()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 110`** (2 nodes): `Return maximum lookback for *interval*.          Args:             interval:`, `.get_max_history()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 111`** (2 nodes): `templates.py`, `Strategy Templates — pre-built system strategies seeded into the database.  Th`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 112`** (2 nodes): `eslint.config.js`, `globals.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 113`** (2 nodes): `useContextPopup.js`, `useContextPopup()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 114`** (2 nodes): `useKeyboardShortcuts.js`, `useKeyboardShortcuts()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 115`** (1 nodes): `Enforce at least 1 uppercase, 1 lowercase, 1 digit, and 1 special character.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 116`** (1 nodes): `Enforce at least 1 uppercase, 1 lowercase, 1 digit, and 1 special character.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 117`** (1 nodes): `Reject non-positive portfolio values.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 118`** (1 nodes): `Allow only known mode values.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 119`** (1 nodes): `Reject state values outside the allowed set.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 120`** (1 nodes): `Reject schedule values outside the allowed set.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 121`** (1 nodes): `Adjust price for slippage.          Entries slip against you (buy higher, sell`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 122`** (1 nodes): `Build a TradeRecord from an open position and its exit.          Args:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 123`** (1 nodes): `Size a position by risking a fixed percentage of the account.          The num`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 124`** (1 nodes): `Size a position by risking a fixed dollar amount.          Args:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 125`** (1 nodes): `Full Kelly criterion — optimal fraction of account to wager.          Formula:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 126`** (1 nodes): `Fractional Kelly — conservative variant.          Multiplies the full Kelly fr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 127`** (1 nodes): `ATR-based position sizing.          Uses Average True Range to set a volatilit`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 128`** (1 nodes): `Fetch the latest quote for *symbol*.          Args:             symbol: Ticke`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 129`** (1 nodes): `Return the list of bar intervals this provider supports.          Returns:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 130`** (1 nodes): `Return the maximum historical lookback for *interval*.          Args:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 131`** (1 nodes): `Remove duplicate timestamps and sort chronologically.          Args:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 132`** (1 nodes): `Check if NYSE is in regular trading hours.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 133`** (1 nodes): `Return a numeric sort key so intervals order from shortest to longest.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 134`** (1 nodes): `setup.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 135`** (1 nodes): `chartStyles.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 136`** (1 nodes): `shared.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 137`** (1 nodes): `vite.config.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `User` connect `Community 1` to `Community 0`, `Community 4`, `Community 5`, `Community 6`, `Community 19`?**
  _High betweenness centrality (0.060) - this node is a cross-community bridge._
- **Why does `RuleContext` connect `Community 2` to `Community 3`, `Community 6`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
- **Why does `RuleAlertData` connect `Community 2` to `Community 3`, `Community 6`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
...
- **Are the 118 inferred relationships involving `PortfolioPosition` (e.g. with `routes/feedback.py — Scoring feedback loop endpoints and background tasks.  Pr` and `Validate the X-Internal-Key header and optional IP restriction.      Raises HT`) actually correct?**
  _`PortfolioPosition` has 118 INFERRED edges - model-reasoned connections that need verification._
