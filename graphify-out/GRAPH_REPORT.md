...
## Summary
- 3246 nodes · 10710 edges · 142 communities detected
- Extraction: 32% EXTRACTED · 68% INFERRED · 0% AMBIGUOUS · INFERRED: 7290 edges (avg confidence: 0.5)
...
Cohesion: 0.02
Nodes (265): _apply_stage_result(), _build_soft_stop_report(), _check_condition(), _check_soft_stops(), evaluate_one_soft_stop(), evaluate_price_alerts(), _fetch_daily_close(), _fetch_price() (+257 more)

### Community 1 - "Community 1"
Cohesion: 0.03
Nodes (207): admin_check(), list_audit_logs(), list_users(), lock_account(), lock_user(), admin.py — Admin-only routes for TickerTap.  Provides endpoints for user and a, Lock or unlock a brokerage account atomically.      Eliminates duplication bet, List all registered users, ordered newest-first.      Args:         db: Async (+199 more)

### Community 2 - "Community 2"
Cohesion: 0.02
Nodes (206): alerts.py — Price alert CRUD endpoints.  Lets users create, list, update, and, AuthOut, create_access_token(), decode_access_token(), hash_password(), login(), auth.py — Core authentication utilities for TickerTap.  Provides password hash, Raise on startup if the JWT secret is still the insecure default.      Called (+198 more)
...
Cohesion: 0.05
Nodes (118): _change_pct_to_score(), _check_article_outcomes(), create_scoring_rules(), export_training_data(), _fetch_outcome_prices(), get_active_rules(), get_outcome_data(), get_watch_tickers() (+110 more)
...
Cohesion: 0.04
Nodes (63): advice_lines(), format_advice_block(), load_investment_rules(), _news_bias(), _position_phase_note(), briefing_advice.py — Template advice from investment_rules.json + book.  No LLM., Two-level stop line using the position's actual stored levels rather     than a, Squeeze/crowding context from FINRA's biweekly short-interest data —     a fresh (+55 more)

### Community 11 - "Community 11"
Cohesion: 0.07
Nodes (44): _mdy_to_iso(), parse_form144_xml(), form144_edgar.py — Parse SEC Form 144 (Notice of Proposed Sale) atom + XML.  An, One proposed-sale notice per filing (unlike Form 4's multi-row shape).      Retu, EDGAR's ownership XML dates are MM/DD/YYYY, unlike Form 4's ISO dates., _already_stored(), get_recent_144_notice(), _parse_date() (+36 more)

### Community 12 - "Community 12"
Cohesion: 0.13
Nodes (35): _deps(), _deps_multi(), MapFetcher, _mapping(), test_144_notice_note_appended_to_sell_telegram_body(), test_8k_note_appended_to_telegram_body(), test_all_new_features_together(), test_beneficial_ownership_note_appended_for_activist_13d() (+27 more)
...
Cohesion: 0.17
Nodes (44): Named watchlist owned by a user for tracking assets without positions., A single asset tracked within a Watchlist., Watchlist, WatchlistItem, Update a watchlist item., Buy an asset from watchlist into a portfolio., Create a new watchlist., Serialised watchlist item. (+36 more)
...
- **Why does `User` connect `Community 1` to `Community 0`, `Community 2`, `Community 5`, `Community 10`, `Community 22`?**
